...
public class JdbcReviewDao extends JdbcUtilities implements ReviewDao {
    ...
    public LinkedList<Review> getRecentReviewForShop(Shop shop) throws Exception {
        ...
        String query = "select R.id, R.global_value, R.quality, R.service, R.description, R.description, R.creation_date, R.creator_id, R.products_id as product_id from reviews R join products P on P.id = R.products_id where P.shops_id = ? AND (select datediff((select now()),R.creation_date)) <= 30";
